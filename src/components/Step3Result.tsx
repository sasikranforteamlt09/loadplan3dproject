...
export default function Step3Result({ plan, onBack }: { plan: Plan; onBack: () => void }) {
  ...
  return (
    <div className="space-y-4">
            ...
            <thead>
              <tr>
                {['ลำดับ', 'พัสดุ', 'ขนาดที่วาง (ย×ก×ส ซม.)', 'ระยะจากหน้าตู้', 'ระยะจากผนังซ้าย', 'ความสูงจากพื้น', 'กก.'].map(h => (
                  <th key={h} className="border border-navy-200 bg-navy-50 p-1.5 text-[13px] font-semibold">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-3 noprint">
...
function Kpi({ v, t, id }: { v: string; t: string; id?: string }) {
  return (
    <div className="lp-kpi">
      <b id={id} className="block text-[21px] leading-tight text-navy-700">{v}</b>
      <span className="text-[12.5px] text-muted">{t}</span>
    </div>
  );
}
